import { db } from "@/db";
import { superBowlSquares, superBowlSquaresConfig, users } from "@/db/schema";
...
export default async function SquaresPage() {
  // Get or create config for current season
  let config = await db
    .select()
  ...
  // Get all squares for current season
  const squares = await db
    .select()
    .from(superBowlSquares)
    .where(eq(superBowlSquares.season, CURRENT_SEASON));
